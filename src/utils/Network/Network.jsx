...
const Network = () => {
  const [isNetworkInfoVisible, setIsNetworkInfoVisible] = useState(true);
  const [isNetworkDetailsVisible, setIsNetworkDetailsVisible] = useState(false);
  ...
  const hideNetworkDetailsAfterDelay = () => {
    setTimeout(() => {
      setIsNetworkDetailsVisible(false);
    }, 2000); // Hide after 2 seconds, not 20
  };

  // Debounced update for network info
  useEffect(() => {
    const updateNetworkInfo = () => {
      checkNetworkSpeed();
      checkConnectionType();
    ...
    const handleConnectionChange = () => {
      setIsNetworkDetailsVisible(true); // Show on network change
      hideNetworkDetailsAfterDelay(); // Hide after a delay
      updateNetworkInfo();
    };
  ...
  // Automatically show network info after 3 seconds on mount
  useEffect(() => {
    const showNetworkInfoTimeout = setTimeout(() => {
      setIsNetworkInfoVisible(true);
      setIsNetworkDetailsVisible(true);
      hideNetworkDetailsAfterDelay();
    }, 3000); // Show after 3 seconds
  ...
  // Memoizing network speed display for better performance
  const speedDisplay = useMemo(() => {
    if (networkSpeed && parseFloat(networkSpeed) > 5) {
      return {
        label: "fast",
      ...
      >
        <div className={styles.network_details}>
          ...
          <div className={styles.base}>
            ...
            <div className={styles.space}>
              <div className={styles.speed}>
                <p className={`${styles.quality} ${styles[speedDisplay.label]}`}>
                  {speedDisplay.label}
                </p>
                <span>{networkSpeed}</span>
              </div>
              <div className={styles.icon}>
                <FontAwesomeIcon icon={speedDisplay.icon} aria-label={speedDisplay.label} />
              </div>
            </div>
          </div>
        </div>
